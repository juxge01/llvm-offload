//===- mlir-fusion-opt.cpp - MLIR Rewrite Driver -----------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Main entry function for mlir-fusion-opt.
//
//===----------------------------------------------------------------------===//
...
public:
  ...
  /// Returns the command line option that may be passed to 'mlir-fusion-opt' to
...
static llvm::cl::opt<bool> dumpArith{
    ...
    llvm::cl::init(false),
    llvm::cl::cat(clCutomPassCategory)};
static llvm::cl::opt<bool> printOffloadFnsOnly{
    "print-offload-fns-only", llvm::cl::desc("offload.target이 부착된 함수의 원문 IR만 출력"),
    llvm::cl::init(false),
    llvm::cl::cat(clCutomPassCategory)};

LogicalResult printOffloadFnBodies(RewritePad &pad, raw_ostream &os) {
  for (Operation &op : *pad.getParsed()) {
    if (auto fn = dyn_cast<func::FuncOp>(&op)) {
      if (fn->hasAttr("offload.target")) {
        const auto &def = pad.getOpDef(fn);
        os << pad.getSourceString(def) << "\n\n";
      }
    }
  }
  return success();
...
LogicalResult autoOffload(RewritePad &pad, raw_ostream &os) {
  ...
  pad.getParsed()->walk([&](Operation *op) {
    ...
    if (!fn->hasAttr("offload.target")) {
      fn->setAttr("offload.target", StringAttr::get(pad.getContext(), "custom-accel"));
      llvm::outs() << "[auto‑offload] " << fn.getName() << " (" << arithCnt << " arith) → custom‑accel\n";

      std::string newPrintedIR;
      llvm::raw_string_ostream ss(newPrintedIR);
      fn.print(ss);  // 갱신된 IR을 직접 print
      pad.replaceDef(def, ss.str());
      // ensure updated IR is flushed back into output
      // pad.replaceDef(def, pad.getSourceString(def));
    }
  });
...
static mlir::RewriterRegistration
    rewriteAutoOffload("auto-offload",
...
int main(int argc, char **argv) {
  ...
  std::string helpHeader = "mlir-fusion-opt";
  ...
  if (rewriter)
    result = rewriter->invoke(*rewriterOr, output->os());

  if (printOffloadFnsOnly)
    result = printOffloadFnBodies(*rewriterOr, output->os());
